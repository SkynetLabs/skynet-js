...
describe(`File API integration tests for portal '${portal}'`, () => {
  ...
  it("Should get existing File API JSON data", async () => {
    ...
    const { data: received } = await client.file.getJSONV2(userID, path);
  ...
  it("Should get existing File API entry data", async () => {
    ...
    const { data: received } = await client.file.getEntryDataV2(userID, path);
  ...
  it("Should get an existing entry link for a user ID and path", async () => {
    ...
    const entryLink = await client.file.getEntryLinkV2(userID, path);
